...
class CedulaMainWindow(QtWidgets.QMainWindow, FORM_CLASS):
    def __init__(self, cveCatas = "0", cond = False, parent=None, CFG=None, UTI = None, cargandoRevision = False, cve_len = 10, si = ""):
        """Constructor."""
        ...
        self.cmbOrientacion.setVisible(False)
        self.leDispPerim.setVisible(False)
        self.leDescripcion.setVisible(False)
        self.btnColinAdd.setVisible(False)
        self.btnColinRemoveOne.setVisible(False)
        self.btnColinRemoveAll.setVisible(False)
        #NUEVO DISENO
        ...
        self.label_18.setText(str(self.si))
    ...
    def showEvent(self, event):
        ...
        #Para calculo de valor catastral
        self.btnCalculo.clicked.connect(self.caluclo_valor)
        ...
        self.comboClase.currentIndexChanged.connect(self.event_cambioClaseCategoria)
        ...
        try:
            ...
            for dcp in construcciones:
                ...
                if len(fracciones) == 0:
                    ...
                    fr['nombre'] = ''
                    fr['nvlUbica'] = ''
                    fr['anioConstruccion'] = dcp['anioConstruccion']
    ...
    def caluclo_valor(self):
        self.consumeWSGeneral(self.CFG.urlCalculoValorCata + str(self.cveCatastral))
        self.cargaInfoValorCatastral()


    def cargaInfoValorCatastral(self):
        data = self.consumeWSGeneral(self.CFG.urlInfoValorCata  + str(self.cveCatastral))
        print('-----------------------------------------------------')
        print(data)
        print('-----------------------------------------------------')
        self.label_217.setText(data['valorUnitario']['codigo'])
        self.label_218.setText(str(data['valorUnitario']['frenteBase']))
        self.label_219.setText(str(data['valorUnitario']['fondoBase']))
        self.label_275.setText(str(data['valorUnitario']['valorCatConst']))
        self.label_274.setText(str(data['valorUnitario']['valorCata']))
        self.label_278.setText(str(data['valorUnitario']['valorCataTerreno']))


        #self.label_269.setText(str(data['totales']['totalsu']))
        #self.label_270.setText(str(data['totales']['totalval']))


        self.label_230.setText(str(data['superficie']['supConstruccion']))
        self.label_229.setText(str(data['superficie']['supTerreno']))
        self.label_226.setText(str(data['superficie']['terrenoTotal']))

        colin = data['construccionesProp']
        self.tableWidget_2.clearContents()
        self.tableWidget_2.setRowCount(len(colin))
        if len(colin) > 0:
            for row in range(len(colin)):
                print(colin[row])
                self.tableWidget_2.setItem(row, 0, QtWidgets.QTableWidgetItem(str(colin[row]['volumen'])))
                self.tableWidget_2.setItem(row, 1, QtWidgets.QTableWidgetItem(str(colin[row]['valor'])))
                #self.tableWidget_2.setItem(row, 2, QtWidgets.QTableWidgetItem(str(colin[row]['sup_const'])))
                self.tableWidget_2.setItem(row, 6, QtWidgets.QTableWidgetItem(str(colin[row]['anio_construccion'])))
                self.tableWidget_2.setItem(row, 3, QtWidgets.QTableWidgetItem(str(colin[row]['codigo'])))
                self.tableWidget_2.setItem(row, 8, QtWidgets.QTableWidgetItem(str(colin[row]['nivel'])))
    ...
    def cargaCedula(self, dataCed):
        ...
        try:
            ...
            self.leNoInterior_2.setText(self.cedula['areaValor'])
    ...
    def deshhabilitaCedula(self):
        ...
        self.btnCancelSelCalle.setEnabled(True)
        self.leNoExteriorAlf.setEnabled(True)
        self.cmbTipoPredio.setEnabled(True)
        self.leNoExteAnt.setEnabled(True)
        self.leNumPredio.setEnabled(True)
        self.cmbRegimenProp.setEnabled(True)
        self.cmbOrientacion.setEnabled(True)
        self.leDispPerim.setEnabled(True)
        self.leDescripcion.setEnabled(True)
        self.btnColinAdd.setEnabled(True)
        self.btnColinRemoveOne.setEnabled(True)
        self.btnColinRemoveAll.setEnabled(True)      
        self.rbtnCopiar.setEnabled(True)
        self.rbtnCortar.setEnabled(True)
        self.cmbDest.setEnabled(True)
        self.btnDelete.setEnabled(True)
        self.cmbClcata.setEnabled(True)
        self.btnProc.setEnabled(True)
        self.btnSubir.setEnabled(True)
        self.btnGuardaImg.setEnabled(True)
        self.leNombre.setEnabled(True)
        self.cmbTipoUsoSuelo.setEnabled(True)
        self.cmbTipoRelieve.setEnabled(True)
        self.cmbFacilComun.setEnabled(True)
        self.leFondo.setEnabled(True)
        self.cmbFormaPredio.setEnabled(True)
        self.cmbUsoSuelo.setEnabled(True)
        self.cmbValorTerr.setEnabled(True)
        self.cmbUsoPredio.setEnabled(True)
        self.leFrente.setEnabled(True)
        self.cmbOrientPredMza.setEnabled(True)
        self.btnDelConstrP.setEnabled(True)
        self.btnAddConstP.setEnabled(True)
        ...
        self.leValConstComunCond.setEnabled(True)
        self.leValConstPrivCond.setEnabled(True)
        self.leSupConstExcCond.setEnabled(True)
        self.leSupConstTotalCond.setEnabled(True)
        self.leValConstExcCond.setEnabled(True)
        self.leValConstTotalCond.setEnabled(True)
        self.leCveCatAntCond.setEnabled(True)
        self.leSupTerrPrivCond.setEnabled(True)
        self.leSupTerrComunCond.setEnabled(True)
        self.leSupTerrExcCond.setEnabled(True)
        self.leSupTerrTotalCond.setEnabled(True)
        self.leValTerrPrivCond.setEnabled(True)
        self.leValTerrComunCond.setEnabled(True)
        self.leValTerrExcCond.setEnabled(True)
        self.leValTerrTotalCond.setEnabled(True)
        ...
        self.twServiciosCalle.setEnabled(True)
        self.twServiciosPredio.setEnabled(True)
        self.twCaracteristicasP.setEnabled(True)
        self.twServiciosCondo.setEnabled(True)
        self.twCaracteristicasC.setEnabled(True)
        self.lePrivadaC.setEnabled(True)
        self.leComunC.setEnabled(True)
        self.lePrivadaT.setEnabled(True)
        self.leComunT.setEnabled(True)
        self.btnBlocDesbloc.setEnabled(True)
        self.btnActualizaInfo.setEnabled(True)
    ...
    def consumeWSConstr(self, cveCatastral, tipoCta = 'P'):
        ...
        if self.cargandoRevision:
            ...
            print(idPredio)
            url = self.CFG.urlReviConst + cveCatastral + '/' + tipoCta + '/' + str(idPredio)
        else:
    ...
    def event_cambioClaseCategoria(self):
        if self.comboClase.count() > 0:
            idUsoConst = self.cmbUsoConstrP.itemData(self.cmbUsoConstrP.currentIndex())
            idClase = self.comboClase.itemData(self.comboClase.currentIndex())
            # -- obtiene categorias
            data = self.consumeWSGeneral(self.CFG.urlCedCategoriasByIdUsoConstAndIdClase + str(idUsoConst) + "/" + str(idClase))
    ...
    def event_cambioUsoConstr(self):
        ...
        if self.cmbUsoConstrP.count() > 0:
            ...
            self.comboClase.clear()

            if lenJson > 0:
                for cate in data:
                    self.comboClase.addItem(str(cate['categoria']), cate['id'])
                    d = {cate['id']: cate}
                    self.cateConstP.append(d)
                    #print(self.cmbCategoriaP.addItem(str(cate['categoria']), cate['id']))
    ...
    def event_cambioCategoria(self):
        ...
        if self.cmbCategoriaP.count() > 0:
            ...
            idUsoConst = self.cmbUsoConstrP.itemData(self.cmbUsoConstrP.currentIndex())
            idClase = self.comboClase.itemData(self.comboClase.currentIndex())
            idCategoria = self.cmbCategoriaP.itemData(self.cmbCategoriaP.currentIndex())
            # -- obtiene categorias
            codigo = self.consumeWSGeneral(self.CFG.urlGetCodigoByUsoClaseCatgoria + str(idUsoConst) + "/" + str(idClase) + "/" + str(idCategoria))
            
            self.lbCveUsoP.setText(codigo['codigo'])
            self.lbValM2P.setText(str(codigo['valorM2']))
