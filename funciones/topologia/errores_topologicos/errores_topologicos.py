...
class errores_topologicos:
    ...
    def localiZarErrorManzana(self):
        try:
            error = self.dockwidget.listWidgetManzana.currentItem().text()
            root = QgsProject.instance().layerTreeRoot()
            group = root.findGroup('ERRORES DE TOPOLOGIA')
            layers = group.findLayers()
            for layer in layers:
    ...
    def localiZarErrorConstruccion(self):
        try:
            error = self.dockwidget.listWidgetConstruccion.currentItem().text()
            root = QgsProject.instance().layerTreeRoot()
            group = root.findGroup('ERRORES DE TOPOLOGIA')
            layers = group.findLayers()
            for layer in layers:
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('errores_topologicos', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/errores_topologicos/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'errores_topologicos'),
            callback=self.run,
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg = errores_topologicosDialog()
